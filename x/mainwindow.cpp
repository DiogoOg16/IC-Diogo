...
void MainWindow::criaGraficoAcc()
{
    criaGraficoAccX();
    criaGraficoAccY();
    criaGraficoAccZ();
...
void MainWindow::criaGraficoGyr()
{
    criaGraficoGyrX();
    criaGraficoGyrY();
    criaGraficoGyrZ();  
...
void MainWindow::criaGraficoMag()
{
    criaGraficoAzimuth();
    criaGraficoPitch();
    criaGraficoRoll();  
...
void MainWindow::on_stopbutton_clicked()
{
    ...
    if(mDataTimer2.isActive()) mDataTimer2.stop();
    if(mDataTimer3.isActive()) mDataTimer3.stop();
...
void MainWindow::on_startbutton_clicked()
{
    ...
    if(pGraficoAcc == 0 && pGraficoGyr == 0 && pGraficoMag == 0)
    {
...
unsigned int testeSemEnvioPlot = 0;
...
void MainWindow::SendInFreq()
{
    ...
    if(ui->AccBox->isChecked()){
        if(load2->obtemAcc_x().size() > 0 && load2->obtemAcc_y().size() > 0 && load2->obtemAcc_z().size() > 0){
            ...
            load2->acc_z_Pop();
            timestampACC++;
            Lifes_Protocol.atualiza_label(load->elem1Label(), timestampACC);
        }
    }
    if(ui->GyrBox->isChecked()){
    ...
    if(ui->MagBox->isChecked()){
        if(load2->obtemAzi().size() > 0 && load2->obtemPitch().size() > 0 && load2->obtemRoll().size() > 0){
                Lifes_Protocol.atualiza_azi(load2->elem1Azi(),timestampMAG);
                Lifes_Protocol.atualiza_pitch(load2->elem1Pitch());
                Lifes_Protocol.atualiza_roll(load2->elem1Roll());
                ...
                timestampMAG++;          
                Lifes_Protocol.atualiza_label(load->elem1Label(), timestampMAG);
        }        
   ...
   /* Lifes_Protocol.atualiza_acc_x(load->elem1Acc_x(),tt);
    Lifes_Protocol.atualiza_acc_y(load->elem1Acc_y());
    Lifes_Protocol.atualiza_acc_z(load->elem1Acc_z());
    Lifes_Protocol.atualiza_gyr_x(load->elem1Gyr_x(),tt);
    Lifes_Protocol.atualiza_gyr_y(load->elem1Gyr_y());
    Lifes_Protocol.atualiza_gyr_z(load->elem1Gyr_z());
    Lifes_Protocol.atualiza_azi(load->elem1Azi(),tt);
    Lifes_Protocol.atualiza_pitch(load->elem1Pitch());
    Lifes_Protocol.atualiza_roll(load->elem1Roll());
    QString acao = load->elem1Label();
    Lifes_Protocol.atualiza_label(acao, timestampMAG);
    QString gps = joj;
        //load->elem1Local();
    Lifes_Protocol.atualiza_gps(gps, timestampMAG);
...
QString contaaa, amostrasPorSegundo1, amostrasPorSegundo2, amostrasPorSegundo3, amostrasPorSegundo4;
int cont1 = 0;
int cont2 = 0;
int cont3 = 0;
int cont4 = 0;
...
void MainWindow::obtemAmostras()
{
    ...
    cont = tt;*/
        ...
        cont3 = timestampMAG;
        c = cont4;
        contaaa =  QString::number(testeSemEnvioPlot);
        logger->write(contaaa);
        testeSemEnvioPlot = 0;
...
void MainWindow::criaGraficoGyrX(){
    ...
    connect(&mDataTimer2, SIGNAL(timeout()), this, SLOT(timerSlotGyrX()));
}
...
void MainWindow::criaGraficoGyrY(){
    ...
    connect(&mDataTimer2, SIGNAL(timeout()), this, SLOT(timerSlotGyrY()));
}
...
void MainWindow::criaGraficoGyrZ(){
    ...
    connect(&mDataTimer2, SIGNAL(timeout()), this, SLOT(timerSlotGyrZ()));
}
...
void MainWindow::criaGraficoAzimuth(){
    ...
    connect(&mDataTimer3, SIGNAL(timeout()), this, SLOT(timerSlotAzimuth()));
}
...
void MainWindow::criaGraficoPitch(){
    ...
    connect(&mDataTimer3, SIGNAL(timeout()), this, SLOT(timerSlotPitch()));
}
...
void MainWindow::criaGraficoRoll(){
    ...
    connect(&mDataTimer3, SIGNAL(timeout()), this, SLOT(timerSlotRoll()));
}
